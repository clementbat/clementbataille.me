<template>
  <div id="appFbBirthdayExtractor">
    <div class="header-container">
      <div class="header">Extract your Facebook friend's birthday dates</div>
    </div>
    <div class="header-container">
      <div class="text">
        <ol>
          <li>
            ...
            >https://www.facebook.com/events/birthdays/</a>
          </li>
          <li>Scroll the page until all the months are loaded</li>
          <li>
            Right click on the page, and save the page as html
            <br />
            <code>Save Page As...</code>
          </li>
          <li>Then upload the page here</li>
        </ol>
      </div>
    </div>
  ...
  methods: {
    ...
    uploadRawHtml(event) {
      ...
      reader.onload = e => {
        ...
        const birthdayObject = htmlInterestingPart
          .match(friendRegExp)
          .reduce((acc, item) => {
    ...
    generateCSV() {
      ...
      this.arrayChecked.forEach(person => {
        ...
        // }
        csv += `Birthday ${person.name}, ${moment(
          person.birthday,
          "DD/MM"
        ).format("MM/DD/YYYY")}\n`;
      });
...
DTEND;VALUE=DATE:${moment(person.birthday, "DD/MM")
          .add(1, "days")
          .format("YYYYMMDD")}
STATUS:CONFIRMED
...
.checkbox-container-lines {
  ...
  border-radius: 8px;
  box-shadow: 0 6px 19px 3px rgba(0, 0, 0, 0.07),
    0 4px 23px 4px rgba(0, 0, 0, 0.06), 0 5px 7px -3px rgba(0, 0, 0, 0.1);
  height: 400px;
  overflow-y: scroll;
}
